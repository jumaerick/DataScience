...
def rollingMeanStd():
    ...
    #     if (order == 0):
    #         st.markdown(
    #             'Since the p-value > %.2f we accept the null hypothesis' % (threshold))
    #         st.markdown('Therefore the timeseries is not stationary')
    #     else:
    #         st.markdown(
    #             'Since the p-value > %.2f the differencing of order %i does not make the time series stationary' % (threshold, order))
